...
public class TancSoStudentite {
    // Konstanti
    public static final int BROJ_INSTANCI = 1000;
    ...
    private Semaphore menSemaphore;  // more than 10 men in the wardrobe at the same time,  you got a problem fam
    private Semaphore womenSemaphore;  // more than 10 women in the wardrobe at the same time, you got a problem fam
    private Semaphore danceSemaphore;  // only 3 couples can dance at the same time, this is providing that
    private Semaphore readyWomanSemaphore;  // tells the gentleman that the lady is ready for dancing
    private static final Object zLock = new Object();  // unnecessary for non-chaotic releasing the readyWomanSemaphore

    public void init() {
        // TODO: da se implementira
        menSemaphore = new Semaphore(10);
        womenSemaphore = new Semaphore(10);
        danceSemaphore = new Semaphore(3);
        readyWomanSemaphore = new Semaphore(0);
    }
    ...
    class Masko extends Thread {
        ...
        public void ucestvo() throws InterruptedException {
            // TODO: da se implementira
            menSemaphore.acquire();
            show.presobleci();
            menSemaphore.release();

            // samo maskoto go povikuva metodot tancuvaj
            readyWomanSemaphore.acquire();
            danceSemaphore.acquire();
            show.tancuvaj();
            danceSemaphore.release();
        }
    ...
    class Zensko extends Thread {
        ...
        public void ucestvo() throws InterruptedException {
            //TODO: da se implementira
            womenSemaphore.acquire();
            show.presobleci();
            synchronized (zLock) {
                readyWomanSemaphore.release();
            }
    ...
    public class Show {
        ...
        public void presobleci() throws RuntimeException {
            ...
            if (t instanceof Masko) {
                synchronized (RANDOM) {
                    brojMaskiGarderoba++;
                    if (brojMaskiGarderoba > BROJ_GARDEROBA) {
