package mk.ukim.finki.kol1_vezbi.september_tribe_dinner;
...
public class SeptemberTribeDinner {
    static Semaphore dinnerPlacesSemaphore;
    static Semaphore dinnerReadySemaphore;
    static Semaphore callChefSemaphore;
    static Semaphore takeMealSemaphore;

    static Lock lock;

    public static void init() {
        dinnerPlacesSemaphore = new Semaphore(4);
        dinnerReadySemaphore = new Semaphore(0);
        callChefSemaphore = new Semaphore(0);
        takeMealSemaphore = new Semaphore(1);
    ...
    public static class TribeMember extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            lock.lock();
            if (state.isPotEmpty()) {
                callChefSemaphore.release();
                dinnerReadySemaphore.acquire();
                lock.unlock();
                ...
                dinnerPlacesSemaphore.acquire();
                state.eat();
                dinnerPlacesSemaphore.release();
            } else {
                takeMealSemaphore.acquire();
                state.fillPlate();
                takeMealSemaphore.release();
    ...
    public static class Chef extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            // Vashiot kod ovde
            callChefSemaphore.acquire();
            state.cook();
            dinnerReadySemaphore.release();
        }
