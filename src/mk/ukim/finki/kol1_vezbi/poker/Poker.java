...
class PokerState extends AbstractState {
    ...
    public void playerSeat() {
        synchronized (PokerState.class) {
            if (emptyTable) {
                emptyTable = false;
                playersAtTable.assertEquals(0, PREVIOUS_ROUND_NOT_FINISHED_POINTS, PREVIOUS_ROUND_NOT_FINISHED);
            }
        }
    ...
    public void dealCards() {
        log(playersAtTable.assertEquals(6, NOT_ENOUGH_PLAYERS_POINTS, NOT_ENOUGH_PLAYERS), null);
        synchronized (PokerState.class) {
            cardsDealed = true;
        }
        Switcher.forceSwitch(5);
    ...
    public void play() {
        synchronized (PokerState.class) {
            if (!cardsDealed) {
                log(new PointsException(CARDS_NOT_DEALED_POINTS, CARDS_NOT_DEALED), null);
            }
        }
    ...
    public void endRound() {
        peopleFinishedPlaying.assertEquals(6, NO_6_FINISHED_PLAYERS_POINTS, NO_6_FINISHED_PLAYERS);
...
class BoundCounterWithRaceConditionCheck {
    ...
    public PointsException checkRaceCondition() {
        return checkRaceCondition(raceConditionDefaultTime, RACE_CONDITION_MESSAGE);
    }
