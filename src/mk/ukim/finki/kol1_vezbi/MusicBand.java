...
public class MusicBand {
    ...
    static Semaphore guitaristsSemaphore;
    static Semaphore guitaristHereSemaphore;
    static Semaphore singersSemaphore;
    static Semaphore readySemaphore;
    static Semaphore playingCompletedSemaphore;
    static Semaphore evaluationDoneSemaphore;

    static Lock lock;
    static int singersCounter;

    public static void init() {
        // TODO: da se implementira
        guitaristsSemaphore = new Semaphore(3);
        singersSemaphore = new Semaphore(2);
        guitaristHereSemaphore = new Semaphore(0);
        readySemaphore = new Semaphore(0);
        playingCompletedSemaphore = new Semaphore(0);
        evaluationDoneSemaphore = new Semaphore(0);
    ...
    public static class GuitarPlayer extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            // TODO: da se implementira
            guitaristsSemaphore.acquire();
            guitaristHereSemaphore.release();  // x3 guitarists releasing
            readySemaphore.acquire();  // x3 guitarists waiting
            state.play();

            playingCompletedSemaphore.release();  // x3 guitarists releasing
            evaluationDoneSemaphore.acquire();  // x3 guitarists waiting
        }
    }
    ...
    public static class Singer extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            // TODO: da se implementira
            singersSemaphore.acquire();

            lock.lock();
            singersCounter++;
            if (singersCounter == 2) {  // second singer is the monitor
                singersCounter = 0;
                lock.unlock();
                guitaristHereSemaphore.acquire(3);
                readySemaphore.release(4);  // 3 guitarists + 1 singer waiting on readySemaphore

                state.play();

                playingCompletedSemaphore.acquire(4);

                state.evaluate();

                evaluationDoneSemaphore.release(4);
                guitaristsSemaphore.release(3);
                singersSemaphore.release(2);
            } else {  // first singer
                lock.unlock();
                readySemaphore.acquire();  // x1 singer waiting
                state.play();

                playingCompletedSemaphore.release();
                evaluationDoneSemaphore.acquire();
            }
        }
    }
